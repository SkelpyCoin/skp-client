...
function getskpTicker(currency){
  request({url: "https://api.coinmarketcap.com/v1/ticker/skelpy/?convert="+currency}, function(err, response, body){
    skpticker[currency]=JSON.parse(body)[0];
  });
}
...
vorpal
  ...
  .action(function(args, callback) {
    ...
    getFromNode('http://'+server+'/peer/status', function(err, response, body){
      ...
      self.delimiter('skp '+args.network+'>');
      callback();
    });
  });
  ...
  .action(function(args, callback) {
    ...
    getFromNode('http://'+server+'/peer/list', function(err, response, body){
      if(err){
        ...
        var spinner = ora({text:"0%",spinner:"shskp"}).start();
        var heights={};
        var delays={};
        var count=0;
      ...
      function(passphrase, seriesCb){
        verifyDelegate(args.name, function(err, response, body) {
          if(body.success) {
            ...
            var transaction = skelpyjs.vote.createVote(passphrase, arr);
...
vorpal
  .command('account send <amount> <recipient>', 'Send <amount> skp to <recipient>. <amount> format examples: 10, USD10.4, EUR100')
  .action(function(args, callback) {
    var self = this;
    if(!server){
      self.log("Please connect to node or network before.");
    ...
    async.waterfall([
      ...
      function(passphrase, seriesCb){
        var skpamount = args.amount;
        var skpAmountString = args.amount;

        if(currency){
          if(!skpticker[currency]){
            return seriesCb("Can't get price from market. Aborted.");
          }
          skpamount = parseInt(args.amount * 100000000 / Number(skpticker[currency]["price_"+currency.toLowerCase()]))
          skpAmountString = skpamount/100000000;
        }

        var transaction = skelpyjs.transaction.createTransaction(args.recipient, skpamount*100000000, null, passphrase);

        self.prompt({
          type: 'confirm',
          name: 'continue',
          default: false,
          message: 'Sending '+skpAmountString+'skp '+(currency?'('+currency+args.amount+') ':'')+'to '+args.recipient+' now',
        }, function(result){
          if (result.continue) {
            return seriesCb(null, transaction);
          }
    ...
    return this.prompt({
      ...
      if (res.passphrase) {
        self.prompt({
          ...
          if (result.continue) {
              var transaction = skelpyjs.delegate.createDelegate(res.passphrase, args.username);
              postTransaction(transaction, function(err, response, body){
              if(body.success){
...
vorpal
  ...
  .action(function(args, callback) {
    ...
    skelpyjs.crypto.setNetworkVersion(network.config.version);
    var passphrase = require("bip39").generateMnemonic();
    self.log("Seed    - private:",passphrase);
    self.log("WIF     - private:",skelpyjs.crypto.getKeys(passphrase).toWIF());
    self.log("Address - public :",skelpyjs.crypto.getAddress(skelpyjs.crypto.getKeys(passphrase).publicKey));
    callback();
  });
...
vorpal
  ...
  .action(function(args, callback) {
    ...
    var spinner = ora({text:"passphrases tested: 0",spinner:"shskp"}).start();
  ...
  .action(function(args, callback) {
    ...
    return this.prompt({
      ...
      if (result.passphrase) {
        ...
        self.log("public key: ",skelpyjs.crypto.getKeys(result.passphrase).publicKey);
        self.log("address   : ",skelpyjs.crypto.getAddress(skelpyjs.crypto.getKeys(result.passphrase).publicKey));
        self.log("signature : ",skelpyjs.crypto.getKeys(result.passphrase).sign(hash).toDER().toString("hex"));
    ...
    return this.prompt({
      ...
      if (result.signature) {
        try{
          ...
          var ecpair = skelpyjs.ECPair.fromPublicKeyBuffer(publickey);
          var ecsignature = skelpyjs.ECSignature.fromDER(signature);
...
var shskpspinner;
vorpal
  .command("shskp", "No you don't want to use this command")
  .action(function(args, callback) {
    var self = this;
    self.log(colors.red(figlet.textSync("shskp")));
    shskpspinner = ora({text:"Watch out, the shskp attack!",spinner:"shskp"}).start();
    callback();
  });
...
vorpal
  ...
  .action(function(args, callback) {
    var time = 0;
    var self=this;
    shskpspinner && shskpspinner.stop();
    ["beavis.zen","meow","bunny","turkey","dragon-and-cow","bong","cheese"].forEach(function(spskp){
      setTimeout(function(){
        self.log(cowsay.say({text:"skp Rocks !", f:spskp}));
      }, time++*1000);
    });
...
vorpal.history('skp-client');

vorpal.log(colors.cyan(figlet.textSync("Skelpy Client","Slant")));
